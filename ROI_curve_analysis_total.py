...
def generate_soil_coords(root_coord, dists=[4,2], dir="X"):
    ...
    for dist in dists:
        if dir == "X":
            coords.append((X+dist, Y+dist, Z)); label_dir.append("X"); labels.append(f"Soil +{round(np.sqrt(2*(dist/2)**2),1)}mm")
            coords.append((X-dist, Y-dist, Z)); label_dir.append("X"); labels.append(f"Soil -{round(np.sqrt(2*(dist/2)**2),1)}mm")
        elif dir == "Y":
            coords.append((X-dist, Y+dist, Z)); label_dir.append("Y"); labels.append(f"Soil +{round(np.sqrt(2*(dist/2)**2),1)}mm")
            coords.append((X+dist, Y-dist, Z)); label_dir.append("Y"); labels.append(f"Soil -{round(np.sqrt(2*(dist/2)**2),1)}mm")
...
def plot():
    ...
    ax.set_title(f"ROI TAC {current_root} {current_dir} - Smoothing: {current_smoothing} - Derivative: {deriv_text}")
